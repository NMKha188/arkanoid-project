...
public class Ball {
    ...
    public void update(double SCREEN_WIDTH, double SCREEN_HEIGHT) {
        ...
        double tiLe = Math.abs((paddle.getWIDTH()/2)/(ball.getCenterX() - paddleCenterX) );
        double goc = maxAngle * tiLe;
        ...
        if(lost==false) {
            ...
            if(dy>0&&((y+radius >=paddle.getY())&&(y+radius <=paddle.getY()+paddle.getHEIGHT())))
            {
                if ((x> paddle.getX() && x < paddleCenterX)) {
                    dx=Math.abs(speed*Math.cos(tiLe*maxAngle))*(-1);
                    dy=Math.abs(speed*Math.sin(tiLe*maxAngle))*(-1);
                }
                else if ( x> paddleCenterX&&x < paddle.getX() + paddle.getWIDTH()) {
                    dx=Math.abs(speed*Math.cos(tiLe*maxAngle));
                    dy=Math.abs(speed*Math.sin(tiLe*maxAngle))*(-1);
                }
